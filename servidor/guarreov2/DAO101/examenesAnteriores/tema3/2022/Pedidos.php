...
$fami = isset($_POST['Familia']) ? $_POST['Familia'] : "";
...
if (isset($_POST['Pedir']) && isset($_POST['Selec'])) {
    $pedido = new Pedido();
    ...
    $cantidades = $_POST['Solicitados'];  // Recogemos el array con las cantidades solicitadas
    
    foreach ($selec as $clave => $valor) {
        $detpedido = new DetPedido();   // Creamos una instancia de la clase DetPedido
...
<body>
    <form name='f1' method='post' action='<?php echo $_SERVER['PHP_SELF']; ?>' enctype='multipart/form-data'>
        <fieldset><legend>Seleccione Cliente</legend>
        ...
        <fieldset><legend>Buscar Producto</legend>
            ...
            <label for='Campo'>Campo</label><input type='text' name='Campo' value='<?php echo $campo ?>'>
            <label for='Orden'>Orden</label>
            <select name='Orden'>
                <option value='ASC' <?php if ($orden == 'ASC') echo 'selected'; ?>>ASC</option>
                <option value='DESC' <?php if ($orden == 'DESC') echo 'selected'; ?>>DSC</option>
            </select>
    ...
    <?php 
    if (isset($_POST['Buscar']) || isset($_POST['Mostrar'])) {
        echo "<fieldset><legend>Resultado de la búsqueda</legend>";
        echo "<form name='f2' method='post' action='Cesta.php'>"; 
        
        if (isset($_POST['Mostrar']) && !empty($campo)) {
            $daoPro->listarOrdenado($campo, $orden);
        } else {
            $daoPro->listFamNom($fami, $nom);
        }
        
        echo "<table border='2'>";
        echo "<th>Selec</th><th>Nombre</th><th>PVP</th><th>Cantidad Pedida</th>";
