...
//comprueba si se ha equivocado 4 veces seguidas
session_start();
...
function Bloqueado($usu) {
    ...
    $consulta = "SELECT Hora, Acceso FROM login WHERE Usuario = :Usuario ORDER BY Hora DESC LIMIT 4";
    ...
    if (TresDeneg($filas)) { // Si tiene cuatro denegaciones está bloqueado
        $bloqueado = time() + $tiempoBloqueo; // Almacenamos la hora hasta la que se encuentra bloqueado
    }
